...
点P $(x,y)$ 的负元是 $(x,-y mod p) =(x,p-y)$ , 即P关于x轴的对称j点
...
$$k=\left\{
\begin{align}
\frac{3x_2+a}{2y_{1}},\ if\ P=\ Q\hspace{100cm}\\
...
### 2.1 群阶
**该点集群的阶即有限域上的椭圆曲线的点个数**

求曲线$y^{2}=x^{3}+ax+b\pmod p$上的点, 可以在$\mathbb{Z}_{p}$上遍历x坐标, 然后检测 $x^3+ax+b$ 是否是模p的二次剩余.  

由[二次剩余](../代数/二次剩余.md)知识, 有限域$\mathbb{Z}_{p}$元素是否为二次剩余的概率约为`50%`, 所以推测点集上的点个数约为 $\lVert E(\mathbb{Z}_{p})\rVert=0.5*p*2+1$, 其中, $p*2$代表$y^2$有一正一负两解, 1代表无穷远点$(0,0)$.
...
> 基于椭圆曲线上的离散对数困难问题, 有[Elgamal公钥加密](../密码学/公钥密码/Elgamal公钥加密.md)的变体

> 离散对数问题定义见 [欧拉定理](../代数/欧拉定理.md)  
